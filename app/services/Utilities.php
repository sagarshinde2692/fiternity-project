...
//     }
    ...
    public function checkinInitiate($finder_id, $finder_data, $customer_id, $customer, $data=null){
		...
		if(!empty($addedCheckin['status']) && $addedCheckin['status'] == 200 || (!empty($schedule_session['status']) && $schedule_session['status'] == 200)){
            ...
            $resp['sub_header_2'] = "Enjoy your workout at ".$finder['title']." ".$finder['location_id']['name'].". Workout for at-least 45 minutes before check-out.<br/> Make sure you check-out by scanning the QR code again to get the successful check-in.<br/> Please note - The checkout window is 45 minutes to 3 hours from the time of your check-in.";
