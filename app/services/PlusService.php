...
class PlusService {
    ...
    public function plusCapture($data){
        //checking existing customers
        $check_existing_order_detail = $this->checkExistingOrder($data);
        $data = !empty($check_existing_order_detail['data']) ? $check_existing_order_detail['data'] : $data;
        $order_exists = $check_existing_order_detail['order_exists'];
        
        //set customer source
        $this->customerSource($data);
        
        //customer details
        $customer_detail = $this->utilities->getCustomerDetail($data);
        if(empty($customer_detail['status']) || $customer_detail['status'] != 200){
            return $customer_detail;
        }
        $data = array_merge($data, $customer_detail['data']);
        
        //plus ratecard details
        $plus_ratecard_details = $this->plusRatecardDetails($data);
        if(!empty($plus_ratecard_details)){
            $data['plus'] = $plus_ratecard_details;
        }
        
        //add reward info
        $this->addRewardInfo($data);
        ...
        //txnid & success added
        $this->txnidAndSuccessUrl($data);
        
        $this->finderServiceDetails($data);
    ...
    public function checkExistingOrder($data){
        $order_exists = false;
        if(!empty($data['order_id'])){
            $order_exists = true;
            $order = Order::find($data['order_id']);
            
            $keys = ['customer_email', 'customer_name', 'customer_phone', 'plus_id'];
            foreach($keys as $key){
        ...
        $returnArr = array();
        $returnArr['data'] = !empty($data) ? $data : null;
        $returnArr['order_exists'] = $order_exists;

        return $returnArr;
    }

    public function customerSource(&$data){
    ...
    public function plusRatecardDetails($data){
        if(!empty($data['plus_id'])){
            $plus = Plusratecards::where('plus_id', $data['plus_id'])->first()->toArray();
        }
        
        return (!empty($plus)) ? $plus : null;
    }

    public function rewardInfo(&$data){
        try{
            //if not corporate add rewards
            if(empty($data['customer_source']) || (!in_array($data['customer_source'], ['sodexo', 'thelabellife', 'corporate']) && (empty($data['corporate_source']) || !in_array($data['corporate_source'], ['generic'])))){
                //add reward info here
            }
        }catch (Exception $e) {
    ...
    public function txnidAndSuccessUrl(&$data){
        $txnid = "";
        $successurl = "";
        $mobilehash = "";
    ...
    public function finderServiceDetails(&$data){
        $data['finder_name'] = Config::get('plus.finder_name');
        $data['finder_slug'] = Config::get('plus.finder_slug');
                
        $data['service_name'] = Config::get('plus.service_name');
        $data['service_id'] = Config::get('plus.service_id');
    ...
    public function allowSession($amount, $customerId, $date = null, $finderId = null, $fromService=null) {
        ...
        if(!empty($passOrder['pass'])) {
                ...
                if($passOrder['pass']['pass_type']=='black'){
                    ...
                    if($schedule_time<strtotime($passOrder['end_date'])){
                        ...
                        if(empty($todaysBooking)) {
                            ...
                            $pass_corporate = !empty($passOrder['pass']['corporate']) ? $passOrder['pass']['corporate'] : null;
            ...
            if (($amount>= $upper_amount && (empty($finder['flags']['forced_on_onepass']) || !($finder['flags']['forced_on_onepass']))) || !$canBook) {
                // over 1000
                return [ 'allow_session' => false, 'order_id' => $passOrder['_id'], 'pass_type'=>$passType, /*'profile_incomplete' => !$profile_completed,*/ 'pass_branding' => $pass_branding, 'pass_corporate'=> $pass_corporate];
            }
            else {
                // below 1001
