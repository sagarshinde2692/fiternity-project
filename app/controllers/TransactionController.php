...
//                        $order->unset('wallet', 'wallet_amount');
    ...
    public function checkoutSummary(){
        ...
        if($ratecard_id && $ratecard_id != ''){
            ...
            if((empty($data['init_source']) || $data['init_source'] != 'pps') && !empty($data['amount_payable']) && (empty($data['coupon_code']) || strtoupper($data['coupon_code']) ==  "FIRSTPPSFREE") && $data['type'] == 'workout session' && (empty($data['customer_quantity']) || $data['customer_quantity'] == 1)){
