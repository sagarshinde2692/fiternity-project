				...
				if(count($trials) > 0){
					...
					foreach ($trials as $trial) {
						...
						if(in_array($this->device_type, ['android', 'ios']) && $this->app_version > '4.4.3'){
							...
							if(!empty($data['pass_order_id']) && $data['time_diff']> 0){ //have to list bookings,  post 2 hour of booking end time   

								$data_new = $this->passService->upcomingPassBooking($customer, $data);

								$data_new = array_merge($data, $data_new);

								$data_new = array_only($data_new, ['icon','title', 'time_diff', 'time_diff_text', 'schedule_date_time', 'current_time', 'schedule_date_time_text', 'payment_done', 'order_id', 'trial_id', 'header', 'workout', 'finder', 'footer', 'direction', 'lat', 'lon', 'user_photo', 'header_text']);
