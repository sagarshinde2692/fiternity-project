...
class CustomerController extends \BaseController {
	...
	public function register(){
        ...
        $validator = Validator::make($data = Input::json()->all(), Customer::$rules);
     	...
     	if ($validator->fails()) {
	        ...
	        return Response::json(array('status' => 200,'message'=>'sucessfully register'),200);
        } 
	...
	public function emailLogin($data){
		...
		if($validator->fails()) {
			return array('status' => 400,'message' =>$validator->errors());  
        }

		$customer = Customer::where('email','=',$data['email'])->where('password','=',md5($data['password']))->first();

		if(empty($customer)){
			return array('status' => 400,'message' => 'Incorrect email and password');
		}
	...
	public function socialRegister($data){
		...
		if ($validator->fails()) {
            $response = array('status' => 400,'message' =>$validator->errors());
        }else{
	...
	public function forgotPasswordEmail(){
		...
		if(isset($data['email']) && !empty($data['email'])){
			...
			if(!empty($customer)){
				...
				return Response::json(array('status' => 400,'message' => 'Customer not found'));
			}
		}else{
			return Response::json(array('status' => 400,'message' => 'Empty email'));
		}
		...
		if(isset($data['password_token']) && !empty($data['password_token'])){
	        ...
	        try{
	        	if(Cache::tags('blacklist_forgot_password_token')->has($password_token)){
	        		return Response::json(array('status' => 400,'message' => 'Token expired'));
	        	}
	            ...
	            if(!empty($decoded)){
	            	...
	            	if ($validator->fails()) {
			           return Response::json(array('status' => 400,'message' =>$validator->errors()));
			        }else{
			        	$password['password'] = md5($data['password']);
	            	...
	            	return Response::json(array('status' => 400,'message' => 'Token incorrect'));
	            }
	            ...
	            return Response::json(array('status' => 400,'message' => 'Signature verification failed'));
	        }catch(Exception $e){
	            return Response::json(array('status' => 400,'message' => 'Token incorrect'));
	        }

	    }else{
	        return Response::json(array('status' => 400,'message' => 'Empty token or token should be string'));
	    }
	}
