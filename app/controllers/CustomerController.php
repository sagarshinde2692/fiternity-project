...
class CustomerController extends \BaseController {
	...
	public function home($city = 'mumbai',$cache = true){
		...
		if(!empty($customeremail)) {
			$passOrder = Order::where('status', '1')->where('type', 'pass')->where('customer_email', '=', $customeremail)->where('start_date','<',new MongoDate())->where('end_date','>=',new MongoDate())->orderBy('_id', 'desc')->first();
			if(!empty($passOrder)) {
				$passPurchased = true;
			}
	...
	public function forceUpdate(){
		...
		$current_version_ios = '5.2.1';

		$last_stable_version_android = 5.28;
