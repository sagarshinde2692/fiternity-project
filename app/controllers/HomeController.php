...
//            return $footer_finders;
    ...
    public function getCities(){
        ...
        if(isset($app_device) && in_array($app_device, ['ios', 'android'])){
            $cites		= 	City::active()->orderBy('order')->whereNotIn('_id',$array)->orderBy("order")->remember(Config::get('app.cachetime'), 'getcitiesapp')->get(array('name','_id','slug'));
...
//        }
    ...
    public function mfp($city = false){
		...
		return $mfpEvents = DbEvent::where("mfp",true)->with(array("city" => function($query){$query->select("name");}) )->get(["name", "slug", "venue","start_date","end_date","gallery","coverimage","city_id", "city"])->toArray();
