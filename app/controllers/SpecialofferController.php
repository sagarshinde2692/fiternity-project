...
public function getMembership($city = 'mumbai', $offertype, $from = '', $size = ''){
	...
	$banners 			= 	Fitmaniahomepagebanner::where('city_id', '=', $city_id)->where('banner_type', '=', $offertype)->take($size)->skip($from)->orderBy('ordering')->get();		
	$location_clusters 	= 	$this->getLocationCluster($city_id);			
	$explore_locations 		= 	$this->exploreLocationClusterOffers($city_id);
	$explore_categorys 		= 	$this->exploreCategoryOffers($city_id);
...
// return $serviceids;
$serviceoffers  			= 	Serviceoffer::where('city_id', '=', $city_id)->where("type" , "=" , $offertype)->whereIn('service_id', $serviceids)->with('finder')->with('ratecard')->get();
...
public function maintainActiveFlag($serviceid = NULL){
	...
	foreach ($ratecardoffers as $key => $offers) {
   	...
   	public function serachFinders (){
   		...
   		$serviceoffers_array  	= 	Serviceoffer::where('city_id', $city_id)->where("type" , 'womens-day')->get(['finder_id'])->toArray();
   		// return $serviceoffers_array;
...
public function exploreCategoryOffers($city_id = 1){
	...
	if(!$explore_category_offers_by_city){
		...
		$categorys_arr     		=  	array('zumba' => '19', 'yoga & pilates' => '1,4',  'gyms' => '65', 'mma & kickboxing' => '3', 'functional / crossfit' => '5,111',  'dance' => '2');
	...
	if(!$explore_locationcluster_offers_by_city){
		...
		foreach ($location_clusters as $key => $value) {
			...
			foreach ($categorys_arr as $k => $v) {
				...
				$serviceoffers_cnt  	= 	Serviceoffer::whereIn("type" ,  ['womens-day'])->whereIn("service_id" , $services)->count();
				$offer = ['category_name' => $servicecategory_name,'category_id' => $servicecategory_id,'cnt'=>$serviceoffers_cnt];
