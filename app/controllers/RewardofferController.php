...
class RewardofferController extends BaseController {
    ...
    public function addUpgradeMembership($data, $ratecard){
        ...
        $wallet = Wallet::active()->where('balance', '>', 0)->where('used', 0)->where('customer_id', $customer_id)->whereIn('service_id', [null, $ratecard['service_id']])->where('valid_finder_id', $ratecard['finder_id'])->where('restricted_for', 'upgrade')->whereIn('duration_day', [null, $duration_day])->first();
