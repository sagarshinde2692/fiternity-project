	    		...
	    		if(isset($_REQUEST['fit_action']) && $_REQUEST['fit_action'] == 'select_extension'){
		    		...
		    		if($extension < 1 || $extension > 99){
				    	...
				    	if($ozonetelNoDetails){
				    		...
				    		$finder_count = Finder::active()->where(function($query){$query->orWhere('membership','!=',"disable")->orWhere('trial','!=',"disable")->orWhere('trial','exists',false)->orWhere('membership','exists',false);})->where('city_id',1)->where('_id',(int)$ozonetelNoDetails->finder->_id)->count();
