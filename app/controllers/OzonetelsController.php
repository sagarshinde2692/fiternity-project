...
class OzonetelsController extends \BaseController {
	...
	public function __construct(
		...
		$bangalore = [3175,3178,3179,3183,3184,3191,3192,3193,3195,3196,3197,3201,3202,3204,3210,3229,3233,3235,3239,3253,3279,3330,3331,3332,3333,3335,3336,3341,3342,3343,3345,3346,3347,3350,3413,3417,3424,3429,3443,3491,3547,3557,3614,3618,3619,3620,3654,3716,3720,3757,3774,3775,3843,3901,3904,3905,3906,3907,3927,3970,3972,3985,3989,4034,4044,4059,4088,4164,4175,4179,4183,4185,4209,4281,4291,4385,4388,4484,4581,4602,4607,4645,4650,4705,4763,4784,4817,4818,4819,4821,4822,4823,4825,4826,4827,4828,4829,4830,4831,5502,5655,5725,5909,5950,5957,5958,5959,5964,5986,5988,6034,6397,6686,6884,6933,6942,6945,6947,6948,6997,7010,7029,7037,7059,7081,7106,7111,7114,7116,7321,7408,7428,7429,7434,7773,7786,7805,7898,8649,8872,8877,9112,9400,9410,9412,9422,9579,9589,9881,9882,9918,9968,10120,10394,10514,10552];
	    		...
	    		if(isset($_REQUEST['fit_action']) && $_REQUEST['fit_action'] == 'select_extension'){
		    		...
		    		if($extension < 1 || $extension > 99){
				    	...
				    	if($ozonetelNoDetails){
				    		...
				    		if($this->jump_start_time < $this->current_date_time && $this->current_date_time < $this->jump_end_time && !in_array($this->sunday,["Sunday","Tuesday"]) && in_array($ozonetelNoDetails->finder->_id, $this->direct_route_to_fitternity_vendor)){
							...
							}elseif($this->jump_start_time < $this->current_date_time && $this->current_date_time < $this->jump_end_time && !in_array($this->sunday,["Sunday","Tuesday"]) && ( in_array($ozonetelNoDetails->finder->_id, $this->jump_finder_ids) || in_array($ozonetelNoDetails->finder->city_id, [4,8,9]))){
		...
		if (isset($_REQUEST['event']) && $_REQUEST['event'] == 'NewCall') {
			...
			if($ozonetelNoDetails){
                ...
                //if(in_array($ozonetelNoDetails->finder->city_id, $city) || in_array($ozonetelNoDetails->finder->_id, $this->jump_finder_ids)) {
                	...
                	if($this->jump_start_time < $this->current_date_time && $this->current_date_time < $this->jump_end_time && !in_array($this->sunday,["Sunday","Tuesday"]) && in_array($ozonetelNoDetails->finder->_id, $this->jump_finder_ids)){
