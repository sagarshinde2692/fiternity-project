	...
	public function scopeGetActiveV1($query, $field_name, $field_value, $finder_id){
		...
		if($gmv1Flag == true){
			...
			return $query->where($field_name, intval($field_value))->where('hidden', false)->orderBy('_id', 'desc')
                    ...
                    ->where(function($query){$query->orWhere('created_at', '>', new DateTime( date("d-m-Y 00:00:00", strtotime('2019-08-31')) ))->orWhere('end_date', '>=', new DateTime( date("d-m-Y 00:00:00", time()) ));})
                    ->get();
		}else{
			Log::info("else");
			return $query->where($field_name, intval($field_value))->where('hidden', false)->orderBy('_id', 'desc')
