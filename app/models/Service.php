			...
			foreach ($ratecardsarr as $key => $value) {
                ...
                if(!empty($value['_id']) && isset($value['_id'])){
					...
					if(count($ratecardoffers) && isset($ratecardoffers[0]['offer_icon'])){
						if(in_array($value['type'], ['membership', 'packages']) && ((isset($this->finder['membership']) &&  $this->finder['membership'] == 'disable') || (isset($this['membership']) && $this['membership'] == 'disable') || (isset($this->finder['flags']) && isset($this->finder['flags']['state']) && in_array($this->finder['flags']['state'], ['temporarily_shut', 'closed'])))){
