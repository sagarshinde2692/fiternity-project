...
class PWAINBackendSDK {
	...
	private function getPublicKey() {
		if( !file_exists ( dirname(__DIR__)."/AmazonPay/config.ini" ) ){
	...
	/* Collapse multiple whitespace characters into a single ' ' and backslash escape '\',
			...
			*/
			...
			private function updateLatencyMetrics($totalTime, $operation){
				$file = dirname(__DIR__).'/AmazonPay/metrics/latencyMetrics.txt';
			...
			private function updateCountMetrics($metrics){
				$file = dirname(__DIR__).'/AmazonPay/metrics/countMetrics.txt';
				$sellerId = $this->config ['merchant_id'];
			...
			private function checkWritePermissions(){
				if(!(is_writable(dirname(__DIR__).'/AmazonPay/metrics'))){
